...
export default function EditSubjectPage() {
  ...
  const { data: subject, isLoading } = trpc.subject.getById.useQuery({ id: subjectId });
  ...
  // Preencher form quando subject carregar
  useEffect(() => {
    if (subject) {
      form.reset({
        name: subject.name,
  ...
  const updateSubjectMutation = trpc.subject.update.useMutation({
    ...
    onError: (error: any) => {
      form.setError('root', {
        message: error.message || 'Erro ao atualizar matéria',
      });
    },
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Editar Matéria</h1>
